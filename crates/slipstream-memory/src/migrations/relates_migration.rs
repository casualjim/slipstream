use crate::edges::Relates;
use arrow::datatypes::{DataType, Field, Schema, TimeUnit};
use lancedb::embeddings::EmbeddingDefinition;
use lancedb::index::Index;
use lancedb::index::scalar::FtsIndexBuilder;
use slipstream_store::{DatabaseCommand, DatabaseOperation, NoData, Result, ToDatabase};
use std::sync::Arc;

/// Migration-specific schema for Relates edge table creation
/// This schema excludes the embedding column so LanceDB can add it via EmbeddingDefinition
fn relates_migration_schema() -> Arc<Schema> {
  Arc::new(Schema::new(vec![
    Field::new("uuid", DataType::Utf8, false),
    Field::new("source_concept_uuid", DataType::Utf8, false),
    Field::new("target_concept_uuid", DataType::Utf8, false),
    Field::new("fact", DataType::Utf8, false),
    Field::new("name", DataType::Utf8, false),
    Field::new("group_id", DataType::Utf8, false),
    Field::new("attributes", DataType::Utf8, false),
    Field::new(
      "interactions",
      DataType::List(Arc::new(Field::new("item", DataType::Utf8, false))),
      false,
    ),
    Field::new(
      "created_at",
      DataType::Timestamp(TimeUnit::Microsecond, None),
      false,
    ),
    Field::new(
      "valid_at",
      DataType::Timestamp(TimeUnit::Microsecond, None),
      true,
    ),
    Field::new(
      "invalid_at",
      DataType::Timestamp(TimeUnit::Microsecond, None),
      true,
    ),
    Field::new(
      "expired_at",
      DataType::Timestamp(TimeUnit::Microsecond, None),
      true,
    ),
    // Note: fact_embedding column will be added by LanceDB via EmbeddingDefinition
  ]))
}

/// Command to run migration for the Relates edge type (formerly RELATES_TO)
/// IMPORTANT: This must run AFTER Concept node migration
pub struct RunRelatesMigration {
  pub embedding_function_name: String,
}

impl DatabaseCommand for RunRelatesMigration {
  type Output = ();
  type SaveData = NoData;

  fn to_operation(&self) -> DatabaseOperation<Self::Output, Self::SaveData> {
    let embedding_function_name = self.embedding_function_name.clone();

    DatabaseOperation::Migration {
      graph_ddl: vec![
        // Create RELATES_TO edge table (Concept -> Concept)
        // Only include properties used in WHERE, ORDER BY, or graph traversal queries
        r#"
                CREATE REL TABLE IF NOT EXISTS RELATES_TO(
                    FROM Concept TO Concept,
                    uuid UUID,
                    source_uuid UUID,
                    target_uuid UUID,
                    group_id STRING,
                    created_at TIMESTAMP,
                    valid_at TIMESTAMP,
                    invalid_at TIMESTAMP,
                    expired_at TIMESTAMP,
                    PRIMARY KEY (uuid)
                )"#,
      ],

      meta_setup: Box::new(move |conn| {
        Box::pin(create_relates_lance_table(conn, embedding_function_name))
      }),
      transformer: Box::new(|_| ()), // No transformation needed
    }
  }
}

/// Internal function to create the relates_edges table in LanceDB
async fn create_relates_lance_table(
  conn: &lancedb::Connection,
  embedding_function_name: String,
) -> Result<()> {
  let table_name = Relates::meta_table_name();

  // Check if table already exists
  let tables = conn.table_names().execute().await?;
  if tables.contains(&table_name.to_string()) {
    tracing::debug!("Relates edges table already exists, skipping creation");
    return Ok(());
  }

  // Create the table with embedding definition
  // This tells LanceDB to automatically generate embeddings for the "fact" column
  // and store them in the "fact_embedding" column using the registered embedding function
  // Note: We use a migration-specific schema without the embedding column
  let schema = relates_migration_schema();

  // Create a minimal batch with dummy data - LanceDB requires at least one batch
  // to create a table with embeddings. We'll create and then immediately delete this row.
  use arrow::array::StringBuilder;
  use arrow::buffer::OffsetBuffer;
  use arrow_array::{
    ArrayRef, ListArray, RecordBatch, RecordBatchIterator, TimestampMicrosecondArray,
  };

  let mut uuid_builder = StringBuilder::new();
  let mut source_builder = StringBuilder::new();
  let mut target_builder = StringBuilder::new();
  let mut fact_builder = StringBuilder::new();
  let mut name_builder = StringBuilder::new();
  let mut group_id_builder = StringBuilder::new();
  let mut attributes_builder = StringBuilder::new();

  // Add dummy data
  uuid_builder.append_value("00000000-0000-0000-0000-000000000000");
  source_builder.append_value("00000000-0000-0000-0000-000000000001");
  target_builder.append_value("00000000-0000-0000-0000-000000000002");
  fact_builder.append_value("__dummy_fact_for_table_creation__");
  name_builder.append_value("dummy");
  group_id_builder.append_value("__dummy__");
  attributes_builder.append_value("{}"); // Empty JSON object

  // Create empty interactions array
  let mut interactions_values_builder = StringBuilder::new();
  let interactions_array = ListArray::new(
    std::sync::Arc::new(arrow::datatypes::Field::new(
      "item",
      arrow::datatypes::DataType::Utf8,
      false,
    )),
    OffsetBuffer::new(vec![0i32, 0].into()),
    std::sync::Arc::new(interactions_values_builder.finish()) as ArrayRef,
    None,
  );

  let created_at = TimestampMicrosecondArray::from(vec![0i64]);
  let valid_at = TimestampMicrosecondArray::from(vec![None::<i64>]);
  let invalid_at = TimestampMicrosecondArray::from(vec![None::<i64>]);
  let expired_at = TimestampMicrosecondArray::from(vec![None::<i64>]);

  // Don't include embedding in the batch - LanceDB will add it via EmbeddingDefinition
  let batch = RecordBatch::try_new(
    schema.clone(),
    vec![
      std::sync::Arc::new(uuid_builder.finish()),
      std::sync::Arc::new(source_builder.finish()),
      std::sync::Arc::new(target_builder.finish()),
      std::sync::Arc::new(fact_builder.finish()),
      std::sync::Arc::new(name_builder.finish()),
      std::sync::Arc::new(group_id_builder.finish()),
      std::sync::Arc::new(attributes_builder.finish()),
      std::sync::Arc::new(interactions_array),
      std::sync::Arc::new(created_at),
      std::sync::Arc::new(valid_at),
      std::sync::Arc::new(invalid_at),
      std::sync::Arc::new(expired_at),
    ],
  )?;

  let batch_iter = RecordBatchIterator::new(vec![batch].into_iter().map(Ok), schema.clone());

  let table = conn
    .create_table(table_name, Box::new(batch_iter))
    .add_embedding(EmbeddingDefinition::new(
      "fact",                   // source column
      &embedding_function_name, // registered embedding function
      Some("fact_embedding"),   // destination column
    ))?
    .execute()
    .await?;

  // Delete the dummy row
  table
    .delete("uuid = '00000000-0000-0000-0000-000000000000'")
    .await?;

  // Create scalar indices for fast lookup
  for col in [
    "uuid",
    "source_concept_uuid",
    "target_concept_uuid",
    "group_id",
    "created_at",
    "valid_at",
    "invalid_at",
    "expired_at",
  ] {
    if let Err(e) = table.create_index(&[col][..], Index::Auto).execute().await {
      tracing::warn!("Failed to create scalar index for {}: {}", col, e);
    }
  }

  // Create fulltext index for text search on fact and name
  for col in ["fact", "name"] {
    if let Err(e) = table
      .create_index(&[col], Index::FTS(FtsIndexBuilder::default()))
      .execute()
      .await
    {
      tracing::warn!("Failed to create fulltext index on {}: {}", col, e);
    }
  }

  // Note: Vector index for fact_embedding will be created later when we have data
  // LanceDB requires data to exist before creating vector indices

  tracing::info!(
    "Created Relates edges table with embedding function '{}' and indices",
    embedding_function_name
  );
  Ok(())
}

#[cfg(test)]
mod tests {
  use super::*;
  use slipstream_store::Database;

  #[tokio::test]
  async fn test_relates_migration() {
    let data_dir = tempfile::tempdir().unwrap().keep();
    let config = slipstream_store::Config::new_test(data_dir);
    let db = Database::new(&config).await.unwrap();

    // First run node migrations - edges require nodes to exist
    use crate::migrations::concept_migration::RunConceptMigration;
    db.execute(RunConceptMigration {
      embedding_function_name: config.embedding.provider.clone(),
    })
    .await
    .expect("Concept migration should succeed");

    // Now run relates edge migration
    let result = db
      .execute(RunRelatesMigration {
        embedding_function_name: config.embedding.provider.clone(),
      })
      .await;
    assert!(
      result.is_ok(),
      "Relates migration should succeed: {:?}",
      result.err()
    );

    // Running again should be idempotent
    let result = db
      .execute(RunRelatesMigration {
        embedding_function_name: config.embedding.provider.clone(),
      })
      .await;
    assert!(
      result.is_ok(),
      "Second migration run should be idempotent: {:?}",
      result.err()
    );
  }
}
